...
class SampledSoftmaxLayer(nn.Module):
    ...
    def __init__(self, output_dim, n_class, n_samples, device):
        """
    ...
    def forward(self, x, y):
        if self.training:
            ...
            samples = torch.empty(L, dtype=torch.int64, device=self.device).fill_(0)
            ...
            samples = torch.empty(len(SampledSoftmaxLayer.all_word_to_column), dtype=torch.int64,
        ...
        if samples.size(0) > A.size(1):
            samples = torch.cat(
                (samples, torch.zeros(samples.size(0) - A.size(1), dtype=torch.int64, device=self.device)), dim=0)
        elif A.size(1) > samples.size(0):
            A = torch.cat((A, torch.zeros((A.size(0), A.size(1) - samples.size(0)), device=self.device)), dim=1)
    ...
    def update_embedding_matrix(self):
        word_inp, chars_inp = [], []
        if self.training:
            columns = torch.zeros(len(self.negative_samples) + 1, dtype=torch.int64, device=self.device)
            samples = self.negative_samples
            for i, word in enumerate(samples):
                if word in self.word_to_column and self.word_to_column[word] < columns.size(0):
                    columns[self.word_to_column[word]] = word
            columns[0] = 0
        else:
            columns = torch.zeros(len(SampledSoftmaxLayer.all_word) + 1, dtype=torch.int64, device=self.device)
...
class CNNSoftmaxLayer(nn.Module):
    def __init__(self, token_embedder, output_dim, n_class, n_samples, corr_dim, device):
    ...
    def forward(self, x, y):
        if self.training:
            for i in range(y.size(0)):
                y[i] = self.word_to_column.get(y[i].tolist())
            samples = torch.empty(len(self.word_to_column), dtype=torch.int64, device=self.device).fill_(0)
            ...
            samples = torch.empty(len(self.all_word_to_column), dtype=torch.int64, device=self.device).fill_(0)
    ...
    def update_embedding_matrix(self):
        ...
        for i in range(len(samples)):
            ...
            if len(word_inp) == batch_size or i == len(samples) - 1:
                sub_matrices.append(self.token_embedder.forward(
                    torch.tensor(word_inp, dtype=torch.int64, device=self.device).view(len(word_inp), 1),
                    None if chars_inp[0] is None else torch.LongTensor(chars_inp).view(len(word_inp), 1,
                                                                                       len(package[1])),
